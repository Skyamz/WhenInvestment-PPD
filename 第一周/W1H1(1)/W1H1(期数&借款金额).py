...
start = time.clock()
LC = pd.read_csv("/Users/LGH/Documents/PycharmProjects/Contest/data/LC.csv",
                 sep=',')  # names=  encoding=   ,usecols=range(10)
...
# 按借款期数来画图
total = data.groupby(LP['期数'])['借款金额'].sum()
payall = data[data['还款状态'] == 3].groupby(LP['期数'])['应还本金'].sum()
Rpayall = payall / total
paypart = data[data['还款状态'] == 4].groupby(LP['期数'])['应还本金'].sum() - data[data['还款状态'] == 4].groupby(LP['期数'])[
    '剩余本金'].sum()
Rpaypart = paypart / total
paynone = data[data['还款状态'] == 2].groupby(LP['期数'])['应还本金'].sum() + \
          data[(data['还款状态'] == 4) & (data['还款日期'] > data['到期日期'])].groupby(LP['期数'])['剩余本金'].sum()
...
ax.set_title('the proportion of different terms')
...
#    for rect in rects:
 ...
 #       ax.text(rect.get_x() + rect.get_width() / 2., height, '%.8f' % float(height),
 #               ha='center', va='bottom')
